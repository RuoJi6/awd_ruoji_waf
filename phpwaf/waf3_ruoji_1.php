...
function mkdirs_data($dir, $mode = 0777)
{
    if (is_dir($dir) || @mkdir($dir, $mode)) {
        return true;
    }
    if (!mkdirs_data(dirname($dir), $mode)) {
        return false;
    }
...
function __construct(){
    ...
    write_attack_log("method".'  ip:'.$_SERVER["REMOTE_ADDR"]);
    }
...
function detect_upload(){
    foreach ($_FILES as $key => $value) {
        if($_FILES[$key]['size']>1){
            ...
            $this->write_access_log_hacker('hacker_Upload');
            $this->write_attack_log("Upload".'  ip:'.$_SERVER["REMOTE_ADDR"]);
...
function write_access_log_probably($pduanb) { 
    $raw = date("Y/m/d H:i:s").'    '; 
    $raw .= $_SERVER['REQUEST_METHOD'].'     '.$_SERVER['REQUEST_URI'].'     '.$_SERVER['REMOTE_ADDR'].'    '.$pduanb.'    '; 
    $raw .= 'POST: '.file_get_contents('php://input')."\r\n";
    $filename = iconv('utf-8', 'gbk', '/tmp/1log/sy.txt');
    $ffff = fopen($filename, 'a'); //日志路径 
    fwrite($ffff, $raw);  
    fclose($ffff);
}

//黑客记录每次大概访问记录，类似日志，以便在详细记录中查找
function write_access_log_hacker($pduanb) { 
    $raw = date("Y/m/d H:i:s").'    '; 
    $raw .= $_SERVER['REQUEST_METHOD'].'     '.$_SERVER['REQUEST_URI'].'     '.$_SERVER['REMOTE_ADDR'].'    '.$pduanb.'    '; 
    $raw .= 'POST: '.file_get_contents('php://input')."\r\n"; 
    $filename = iconv('utf-8', 'gbk', '/tmp/1log/hacker_re.txt');
    $ffff = fopen($filename, 'a'); //日志路径 
    fwrite($ffff, $raw);  
    fclose($ffff);
}
...
function write_access_logs_detailed(){
    ...
    $raw .= 'POST: '.file_get_contents('php://input')."\r\n"; 
    $filename = iconv('utf-8', 'gbk', '/tmp/1log/xxfw.txt');
    $ffff = fopen($filename, 'a'); //日志路径 
    ...
    for($i=0;$i<strlen($str);$i++){
        ...
        if($ascii>126 || $ascii < 32){ //有中文这里要修改
            if(!in_array($ascii, array(9,10,13))){
                $this->write_access_log_hacker('hacker_interrupt');
                write_attack_log("interrupt".'  ip:'.$_SERVER["REMOTE_ADDR"]);
            }else{
...
function filter_attack_keyword($str){//headers会产生误报
    if(preg_match("/select\b|insert\b|update\b|sleep\b|drop\b|and\b|delete\b|dumpfile\b|outfile\b|load_file|rename\b|floor\(|extractvalue|updatexml|name_const|multipoint\(/i", $str)){
        $this->write_access_log_hacker('hacker_sqli');
        $this->write_attack_log("sqli".'  ip:'.$_SERVER["REMOTE_ADDR"]);
    }
    //文件包含的检测
    if(substr_count($str,$_SERVER['PHP_SELF']) < 2){
        $tmp = str_replace($_SERVER['PHP_SELF'], "", $str);
        if(preg_match("/\.\.|.*\.php[35]{0,1}/i", $tmp)){ 
            $this->write_access_log_hacker('hacker_LFI/LFR');
            $this->write_attack_log("LFI/LFR".'  ip:'.$_SERVER["REMOTE_ADDR"]);
        }
    }else{
        $this->write_access_log_hacker('hacker_LFI/LFR');
        $this->write_attack_log("LFI/LFR".'  ip:'.$_SERVER["REMOTE_ADDR"]);
    ...
    if(preg_match("/base64_decode|eval\(|assert\(|file_put_contents|fwrite|curl|system|passthru|exec|system|chroot|scandir|chgrp|chown|shell_exec|proc_open|proc_get_status|popen|ini_alter|ini_restorei/i", $str)){
        $this->write_access_log_hacker('hacker_EXEC');
        $this->write_attack_log("EXEC".'  ip:'.$_SERVER["REMOTE_ADDR"]);
    }
    if(preg_match("/flag/i", $str)){
        $this->write_access_log_hacker('hacker_flag');
        $this->write_attack_log("GETFLAG".'  ip:'.$_SERVER["REMOTE_ADDR"]);
    }
...
function write_attack_log($alert){
    ...
    $filename = iconv('utf-8', 'gbk', '/tmp/1log/hacker_data.txt');
    $ffff = fopen($filename, 'a'); //日志路径 
    fwrite($ffff, $data);  
    fclose($ffff);
    if(stripos($alert,'GETFLAG') !== false){
        echo "flag{df787897897f8d878d9789789g789g789f}"; //如果请求带有flag关键字，显示假的flag。（2333333）
    }else{
        sleep(3); //拦截前延时3秒
    }
    exit(0);
